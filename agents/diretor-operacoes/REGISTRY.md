...
**Gestor de Tráfego existe desde a v1.0.0.** Campanha, orçamento, conjunto, segmentação,
leitura de métrica e diagnóstico de aquisição têm dono. Duas ressalvas que continuam valendo:
ele **não escreve a copy do anúncio** (isso é `copywriting.meta_ads`, do Copywriter) e só
**executa** na plataforma com acesso autorizado e `guardrails.md` preenchido — sem isso,
analisa e recomenda, não publica.
